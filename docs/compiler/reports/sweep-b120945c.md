...
| N17 | regressions/N17/repro.kf | exit 1 | INVALIDO (repro usa `fn` — PARSE085; nao confirma nem nega o bug) | repro.kf:1:4: error: 'fn' is a reserved word (Kof has no function keyword); declare as 'Type name(...) { }' or 'name(...): Type { }' [PARSE085] |
| N13 | regressions/N13/repro.kf | exit 0 | fix (1) | NativeBackend: assembling /tmp/kof-run-3185408907050988836/Default/Main.s 1 |
| N12 | regressions/N12/repro.kf | exit 0 | duvidoso (saida real=9; esperado antigo=6 — reconfirmar no ledger antes de confiar) | NativeBackend: assembling /tmp/kof-run-8818627246883317404/Default/Main.s 9 |
...
| N18u | build/sweep/n18full.kf | exit 1 | INVALIDO (TU gerado pre-port-0.4.0: PARSE085 em linha 7371; re-gerar e re-rodar) | n18full.kf:7371:4: error: 'fn' is a reserved word (Kof has no function keyword); declare as 'Type name(...) { }' or 'name(...): Type { }' [PARSE085] 0 passed, 1 failed |
| N19u | build/sweep/n19u.kf | exit 1 | INVALIDO (mesmo motivo N18u: TU stale pre-port) | n19u.kf:7371:4: error: 'fn' is a reserved word (Kof has no function keyword); declare as 'Type name(...) { }' or 'name(...): Type { }' [PARSE085] 0 passed, 1 failed |
...
| N9 | regressions/N9/repro.kf | exit 1 | INVALIDO (repro usa `fn` — PARSE085; status FIXED do ledger vem das suites portadas, deste sweep nao) | repro.kf:1:4: error: 'fn' is a reserved word (Kof has no function keyword); declare as 'Type name(...) { }' or 'name(...): Type { }' [PARSE085] |
...
| N1 | regressions/N1/repro.kf | exit 1 | INVALIDO (repro usa `fn` — PARSE085) | repro.kf:5:4: error: 'fn' is a reserved word (Kof has no function keyword); declare as 'Type name(...) { }' or 'name(...): Type { }' [PARSE085] |
...
> Retifica (mesma data): linhas marcadas INVALIDO compilam repros pré-0.4.0 (`fn` reservado) ou TUs stale; **nao confirmam** os status. Ledger nao foi alterada por este sweep.
