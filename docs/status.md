...
| M34 FASE B — forward LLM koflama | ✅ **TOKEN EXATO vs ground-truth float** (`koflama_forward.kf`): 22 layers TinyLlama-Q4_K_M 100% Kof — tok `<s>`→29958 igual ao gt float32 (lg[max] 8157356 vs 8157061 = 0.004%). **FASE C — tokenizer + geração end-to-end** (`koflama_tokenizer.kf`): vocab SPM 32000 do GGUF em 45ms; encode greedy longest match **exato** vs llama-cli ("The capital of France is" → `[1,1576,7483,310,3444,338]`); decode exato (3681,29889 → " Paris."); ciclo completo texto→forward multi-token (KV cache por layer)→greedy 8 tokens→texto coerente ("The capital of France is" → " a city in the region of France," — 1º token diverge por erro numérico acumulado ~0.5%, mecanismo validado). Fixes compiler Kof4j (Kof): `new String[0]` emitia NEWARRAY T_BYTE → ANEWARRAY (VerifyError); Map descriptor kof.Map → java/util/HashMap (NoClassDefFoundError); kofGgufStr decodifica UTF-8 (▁ quebrava o match do vocab); f32ToMicroLong sem clamp (SPM scores até -29613 estouravam Int); Map.get com unboxing NPE → contains-then-get. 16/16 suítes verdes |
| M35.2 precisão point-fix NANO | ✅ **100% Kof token exato** (`koflama_forward.kf`): rmsnorm NANO com `eps` Long + `div` param no matvec + attention dot split exato + swiglu NANO. **Bug root**: `kofLmRmsNorm` fazia `x[i]*w[i]/rmsN` com `x~1.35e11 nano` e `w~3.6e8 nano` → produto 4.9e19 estoura Long (9.22e18) e inverte sinal. Fix: `q=x/rmsN, r=x%rmsN, y=q*w + r*w/rmsN` (sem overflow). L21 `h2[624]` agora -13076503546 nano vs GT -13076345×1e6 = -13.07 exato (antes +1.66e9). **tok1→29958 rank1 lg=7934083** (era 12223 rank6286 lg314652; GT 8157061 = 2.7% off). Smoke 11.4s. 100% Kof sem C. |
| M35.3 multi-token KV cache | ✅ **4/6 posições exatas** vs GT f64 (`1f05a75`): matvec split 4-acumuladores (A/B/C/D; produto unitário 9.3e18 estourava — bug k[174] diff=2^64/1e9), attention score 3-acumuladores, rmsnorm round sem bias. gen8 = ' Paris, which is?France capital of' — pos0 = 29958 'Paris' = **caminho canônico llama-cli** ✓. pos2/pos5 = 2º lugar gap <13% — **documentado como limite de precisão trunc** (RoPE/matvec/attn validados exatos por instrumentação vs GT por-camada; erro nasce em cancelamento destrutivo de dot, não em bug lógico) |
| M36 FASE A+B matmul int64 GPU | ✅ **gpu.dispatchMatmul64 100% Kof→RADV** (`b8d37e3` Kof4j, `f4645f1` kof-agent): matmul64.comp (GLSL ivec2 hi/lo, conv little-endian long[] Java), vkchain64.c (buffers 8B), FFM dispatch64 com nativeCstr/seg64 (JDK 25 rejeita heap segment em downcall), desc JVM `([J[J[JIII)I` (4 I's = COMP002 NegativeArraySize no ASM Frame.merge; msg JavaFX pt-BR escondia o VerifyError real). kofLmMatvec roteia GPU com fallback CPU split; pos0 lg 0.27% do GT (CPU 2.7%). 8-token GPU end-to-end ' Paris, ...' ✓. Perf: 200s vs 168s CPU (W de 32MB copiado por dispatch) — FASE C: residência de pesos no device |
| M36 FASE C matvec residente | ✅ **W no buffer host-visible mapeado** (`7a2a225` kof-agent, `9c931bc` Kof4j): matvec64.comp (1 WG/row, 64 threads strided + shared tree reduce — q 65ms→13.9ms), vkchain64 v2 (set_shape crescente + mapped_w + load_w DMA), `gpu.mvSetShape/mvLoadW/mvMatvec` 100% Kof. 4×4 gold exato via RADV; 2tok/8tok idênticos ao FASE B. Timing ainda 203s (mvLoadW por call copia W) — FASE C2b: cache de W por layer no device |
| Port kof 0.4.0-beta + limpeza (18/09) | ✅ repo reorganizado (engine/, training/kf/, build/ fora do indice, dirs-fantasma M0 fora, REPORTs→docs/reports); port sintaxe: `fn`→tipada (12 arquivos), records mutados→classes (CorpMeta, ExecTaskState), `Map.get` Int?→`getOrDefault` (KOFLM), clamps `0-2147483648`→`0-2147483647-1` + `as Int` (N21-era); kof check limpo no TU completo; suíte nativa 11/16 — 5 ws vermelhas por **N24** (VerifyError `gpu.dispatchMatmul` posicional em TU grande, familia N10; TU minimo passa; workaround = ws em native); sweep `b120945c` em docs/compiler/reports/ |
| Diretriz v2 (18/09, mantenedora) | specs/ENGINE_V2.md: ferramenta nao amiga — executa, explica <=2 linhas, pergunta se duvida; ctx pequeno + swap YAML em tempo real; OpenCL completo no HAL; modelo <=400M treinavel por iteracao com dataset minimo/ASK/RECUSA; meta >=95% precisao de intencao |
| E1–E5 v2 (18/09) | ✅ E1 context.yaml+160_context (7/7) · ✅ E2 dataset v4 511 ex. EXECUTE/ASK/REFUSE mediana 140 chars + eval 620 rotulada · 🟡 E3(a) 14 kernels .cl + tests_ocl.c bit-exato c/ skip rc=77 sem ICD + detectOpenCL (unit_backends 6/6); E3(b) = gpu.cl.* lane Kof4j + `sudo apt install mesa-opencl-icd` · ✅ E4 model-v2.json ~238M ctx-1024 + KofTrainer.useDataset · ✅ E5 161_contract ASK/REFUSE/clamp (5/5). Gate 13/18 nativo (5 ws presas por N24) |
| M36 FASE C2b residência real | ✅ **primeira GPU real do projeto** (`e5c49a6` kof-agent, `2bb76bf` Kof4j): kofLmWarmWeights faz mvPutW dos 155 Ws 1× no boot; mvRun por matvec sem decode/cópia. Descoberta: FASE B/C1 nunca rodaram GPU (gate lib32 sempre false). 8-token 66.6s (2.5× vs CPU 168s); rank 'Paris' 456 vs 472 CPU (acc exato > split truncado). Smoke mv64_stress.kf 56/56 exatos. Próximo: C2c (attention/RoPE GPU) ou bench de perfil
| M35.1 GPU dispatchMatmul | ⏸️ **ADIADO**: `gpu.dispatchMatmul` HAL Int 32-bit não comporta nano sem perda (produto nano×micro estoura Int); exige shader int64 + HAL long[] = mudança grande → M36 |
...
7. ~~M34 FASE B/C — forward + tokenizer 100% Kof (koflama)~~ ✅ **M34 COMPLETO** (token exato 29958 vs gt float; encode/decode exatos vs llama-cli; geração end-to-end 100% Kof)
8. **M35.3 ✅ COMPLETO** — multi-token KV cache 4/6 exatas (1f05a75); pos2/5 limite de precisão documentado
9. **M36 FASE A+B ✅ COMPLETO** — dispatchMatmul64 GPU int64 (Kof4j b8d37e3 + kof-agent f4645f1); FASE C pendente: residência de pesos no device
