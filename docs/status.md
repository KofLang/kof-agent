...
7. **M34 FASE B — transformer forward 100% Kof (koflama)**: ligar koflama_gguf (GGUF+dequant) ao GraphExecutor (embed→rmsnorm→attn+RoPE+KV→SwiGLU→lmhead) sobre tensorMatMul/gpu.dispatchMatmul; comparar dequant com ggml dequantize_row_q4_K de referência. ~~descartar 160_llm_subprocess.kf~~ ✅ removido (renome p/ koflama concluído 01/09: koflama_gguf.kf + koflama_forward.kf + kof_f16.kf, símbolos kofGguf*/kofLm*, --with-llm removido do build.sh)
